...
void B_Tree::backup() {
	...
	string filename("Btreegraph.txt");
	fstream file_out;

	file_out.open(filename, std::ios_base::out);
	if (!file_out.is_open()) {
		cout << "No se encuentra el archivo " << filename << endl;
	}
	else {
		if (!root) return;
		Queue q;
		...
		file_out << endl << endl << endl << "____________________________________________ Tree ____________________________________________" << endl << endl;
		file_out << "Nivo " << level << ":" << endl;
		...
		while (!q.is_empty()) {
			...
			//print_array(info.node_ptr->K, info.node_ptr->k);
			file_out << "[ ";
			for (int i = 0; i < info.node_ptr->k; i++) {
				file_out << info.node_ptr->K[i];
				if (i + 1 != info.node_ptr->k) file_out << " | ";
			}
			file_out << " ]";
			file_out << "   ";
			int i = 0;
		...
		file_out << endl << endl << endl << endl << "___________________________________________________________________________________________________" << endl << endl;
	...
	char ch;
	original = fopen("Btreegraph.txt", "r");
	backup = fopen("recovery.txt", "w");
